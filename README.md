...
- **多渠道支持**：通过 Channel Registry 统一适配 OpenAI、Gemini 等模型服务
...
- **插槽与插件系统**：`Framework.SLOTS`、`Framework.registerPlugin()`、`Framework.unregisterPlugin()`、`Framework.setPluginEnabled()` 提供统一的插件注册和管理机制
- **视图模式与主视图切换**：`Framework.setMode()` 允许在"聊天"等模式与自定义主视图之间切换
- **右侧面板控制**：`Framework.setDefaultRightPanel()`、`Framework.setCustomPanel()` 支持动态面板内容管理
- **底部抽屉**：`Framework.showBottomSheet()`、`Framework.hideBottomSheet()` 提供模态对话框支持
- **消息渲染管线**：`Framework.addMessage()`、`Framework.updateLastMessage()`、`Framework.finalizeStreamingMessage()` 处理消息的添加、更新和最终渲染

Header / 输入区等核心 UI 由 Framework 负责创建，并在中间区域预留多个插件插槽（`HEADER_ACTIONS`、`INPUT_TOP`、`MESSAGE_FOOTER` 等），供内置和外部插件挂载控件。

### 2. 业务核心（IdoFront Core）

业务初始化入口为 [`window.IdoFront.init()`](edge-extension/scripts/ido-front/main.js)，由浏览器扩展页面或 Web 入口在加载时调用，注入 Framework 实例：
...
- **渠道注册中心**：[`channel-registry.js`](edge-extension/scripts/ido-front/channels/channel-registry.js) 提供注册和调用各类模型渠道的统一入口（如 `openai`、`gemini` 等）
- **OpenAI 渠道**：[`openai-channel.js`](edge-extension/scripts/ido-front/channels/openai-channel.js) 负责适配 OpenAI / 兼容协议 API，支持流式输出与 `reasoning_effort` 等高级参数
- **Gemini 渠道**：[`gemini-channel.js`](edge-extension/scripts/ido-front/channels/gemini-channel.js) 负责适配 Google Gemini
...
2. **Channel 插件**：注册新的模型调用渠道
3. **主视图插件**（规划中）：接管整个主内容区，例如图片 gallery 视图

### 📖 完整开发文档

**强烈推荐阅读**：[**外部插件开发指南**](docs/external-plugin-guide.md) 提供了完整的插件开发教程，包括：

- 🚀 快速开始：创建第一个插件
- 📚 完整 API 文档：Framework API、IdoFront API、Channel Registry API
- 🎯 开发规范与最佳实践
- 💾 配置存储与状态管理
- 🐛 调试技巧与常见问题
- 📦 完整代码示例
...
所有插件都挂载到某个插槽（Slot）上，由 `Framework.SLOTS` 预定义：
...
注册插件时，需要提供唯一 id 和渲染函数 / 对象定义，通过 `Framework.registerPlugin()` 完成：
...
内置插件集中定义在 [`core-plugins.js`](edge-extension/scripts/ido-front/plugins/core-plugins.js)，并在初始化时统一注册：
...
1. 在沙箱页面中执行插件脚本
2. 插件脚本通过全局 `Framework` 和 `IdoFront` API 注册自己
...
- 插槽常量：`Framework.SLOTS.HEADER_ACTIONS`
- 注册函数：`Framework.registerPlugin()`
- UI 工具：`Framework.ui.createIconButton()`
- 消息写入：`Framework.addMessage()`

外部插件的导入与管理流程详见 [`docs/external-plugin-guide.md`](docs/external-plugin-guide.md) 第 4 节"导入步骤"。

### 4. 外部 Channel 插件概览

除了 UI 插件，插件还可以注册新的渠道类型，供 IdoFront 统一调用。例如：

- 在沙箱中调用 `channelRegistry.registerType()` 注册新的渠道类型
- 由 Loader 把该渠道暴露给主应用，出现在 Channel 列表中
...
- **完整开发指南**：[`docs/external-plugin-guide.md`](docs/external-plugin-guide.md) - 包含完整 API 文档、开发规范和示例
- **架构设计文档**：[`docs/external-plugin-plan.md`](docs/external-plugin-plan.md) - 外部插件系统的设计思路
- **UI 插件示例**：[`examples/external-plugins/hello-panel/plugin.js`](examples/external-plugins/hello-panel/plugin.js)
- **Channel 插件示例**：[`examples/external-plugins/skugemini-channel/plugin.js`](examples/external-plugins/skugemini-channel/plugin.js)
...
本 README 仅作为 IdoFront 的整体概览与插件编写速查入口，更细节的接口说明与演进规划请直接查阅：

- [`docs/`](docs) 目录下的详细设计文档
- [`edge-extension/scripts/ido-front/`](edge-extension/scripts/ido-front) 目录下的源码与注释
- [`examples/external-plugins/`](examples/external-plugins) 目录下的插件示例

---

**IdoFront** - 让大模型前端开发更加模块化、可扩展、易维护！